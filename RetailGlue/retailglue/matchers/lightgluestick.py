...
LIGHTGLUESTICK_DEFAULT_CONF = {
    'name': 'two_view_pipeline',
    'use_lines': True,
    'extractor': {
        'name': 'wireframe',
        'point_extractor': {
            'name': 'superpoint', 'trainable': False, 'dense_outputs': True,
            'max_num_keypoints': 2048, 'force_num_keypoints': False,
        },
        'line_extractor': {
            'name': 'lsd', 'trainable': False, 'max_num_lines': 250,
            'force_num_lines': False, 'min_length': 15,
        },
        'wireframe_params': {'merge_points': True, 'merge_line_endpoints': True, 'nms_radius': 3},
    },
    'matcher': {'name': 'lightgluestick', 'depth_confidence': -1.0, 'trainable': False},
    'ground_truth': {'from_pose_depth': False},
}


def _build_conf(max_keypoints, max_lines, use_lines):
    conf = copy.deepcopy(LIGHTGLUESTICK_DEFAULT_CONF)
    conf['use_lines'] = use_lines
    conf['extractor']['point_extractor']['max_num_keypoints'] = max_keypoints
    conf['extractor']['line_extractor']['max_num_lines'] = max_lines
    return conf


def _worker(conn, pair_images, max_keypoints, max_lines, use_lines, ransac_threshold, max_edge):
    try:
        import torch
        from lightgluestick import TwoViewPipeline
        from lightgluestick.utils import numpy_image_to_torch, batch_to_np

        conf = _build_conf(max_keypoints, max_lines, use_lines)
        pipeline = TwoViewPipeline(conf).to('cpu').eval()

        batch_src, batch_tgt = [], []
        for img0, img1 in pair_images:
            pts0, pts1 = _match_pair(pipeline, img0, img1, use_lines, ransac_threshold, max_edge)
...
def _match_pair(pipeline, img0, img1, use_lines, ransac_threshold, max_edge):
    import torch
    from lightgluestick.utils import numpy_image_to_torch, batch_to_np
...
class LightGlueStickMatcher:
    def __init__(self, device='cpu', max_keypoints=2048, max_lines=250,
    ...
    def inference(self, image_pairs):
        ...
        proc = ctx.Process(target=_worker, args=(
